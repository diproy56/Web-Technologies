...
function addProduct($data){
    $conn = db_conn();
    $selectQuery = "INSERT INTO `product_table`(`Name`, `Buying Price`, `Selling Price`, `Display`)  VALUES (:name, :buyingPrice, :sellingPrice, :display)";
    try{
...
function updateProduct($id, $data){
    $conn = db_conn();
    $selectQuery = "UPDATE `product_table` set `Name` = ?, `Buying Price` = ?, `Selling Price` = ?, `Display` = ? where `ID` = ?";
    try{
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
