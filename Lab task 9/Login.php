...
<body>
    ...
    <form class="wraper" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
      ...
      <hr class="hr_tag">
		...
		<div>
			<div class="Content">
			<input type="text"  id="user_name" placeholder="Enter User Name" name="user_name" class="input_holder" onkeyup="check()" onblur="check()" onclick="check()" value="<?php if(isset($_COOKIE['user_name'])) {echo $_COOKIE['user_name'];} ?>" >
				<br><span class="error" id="usernameerr"><?php echo $Usernameerr;?></span>
			</div>

			<div class="Content">
			<input type="password" id="password" placeholder="Enter Password" name="password" class="input_holder" onkeyup="checkPassword()" onblur="checkPassword()" onclick="checkPassword()" value="<?php if(isset($_COOKIE['password'])) {echo $_COOKIE['password'];} ?>">
